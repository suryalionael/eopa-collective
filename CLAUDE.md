...
- **Technical architecture:** a Next.js (App Router, TypeScript) static export, deployed via GitHub Actions to GitHub Pages, which serves the custom production domain `https://eoperformancecollective.ca/` (DNS at GoDaddy) — see `docs/ARCHITECTURE.md` and `docs/DEPLOYMENT.md`. This was previously (wrongly) documented as Bluehost; that error caused a real production outage — see `docs/DEPLOYMENT.md`'s incident section before touching `next.config.ts`'s `basePath`/export settings or the deploy workflow. Reuse this architecture; don't introduce a new framework, dependency, or server-side capability without an explicit, approved reason.
