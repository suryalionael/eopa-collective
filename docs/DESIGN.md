...
**Gap to flag:** the mandated 10-page list (see `docs/CONTENT.md`) includes "Get Involved" as a standalone page. Source A folds "Get Involved" into the bottom of `Membership.dc.html` as a section, matching how the Content PDF actually orders it (GET INVOLVED appears as its own top-level heading, between MEMBERSHIP and TEAM, in the source PDF — not nested under Membership). **NEEDS VERIFICATION / decision required:** should "Get Involved" become its own route with its own URL, per the mandated page list, splitting it out of `Membership.dc.html`? This document does not decide that — flag it for the user/Implementation Engineer before Phase 6.
