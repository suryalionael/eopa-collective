...
2. **Where no real photo exists, solve the section with typography and composition — do not represent the gap with a placeholder box.** Revised after a visual-recomposition pass (see `docs/VISUAL_RECOMPOSITION.md`): an early implementation used a grey dashed placeholder box for every missing photo, and it became one of the site's weakest elements — a large, inert, empty object, worst at mobile widths where it could dominate a page (Team) while showing nothing. The absence of a photo is allowed to just be absent.
3. **The 7 supplied rural landscape photos are not a general-purpose accent to distribute across pages "for visual interest."** A first pass used six of them decoratively (one per page) with no stated reason any specific photo belonged next to its specific paragraph, and a creative-direction review correctly flagged all six as forced. Use a photo only where there's a real, statable reason — as implemented, that narrowed to two placements: Home's hero (an establishing view of the region, at real scale) and About's "Where We Stand" (a quiet, real place reinforcing the honest admission of smallness). Never mislabel or caption any of them as performance, rehearsal, or artist photography, since none of them are that.
