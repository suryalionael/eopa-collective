...
**A working Next.js static-export implementation of all 10 required pages plus 4 policy pages now exists** — see `docs/ARCHITECTURE.md` for the technical inventory and `docs/IMPLEMENTATION.md` for what was built and what remains open (real photography, confirmed contact details, a real refund policy, a confirmed domain). It builds, typechecks, and lints cleanly, and deploys as static files via GitHub Actions to GitHub Pages (serving the custom production domain `https://eoperformancecollective.ca/`) per `docs/DEPLOYMENT.md` — an earlier version of this document wrongly described this as a Bluehost deployment, which caused a real production outage (see `docs/DEPLOYMENT.md`'s incident section). The original source material remains in the repository for provenance and as the copy/design source of truth:
...
- **Technical framework is decided and implemented:** Next.js static export, deployed via GitHub Actions to GitHub Pages at the custom domain `https://eoperformancecollective.ca/` — see `docs/ARCHITECTURE.md`. Changing this would be a significant, explicit decision, not a routine one.
